...
namespace TheBadPlaceExecutor.IPC
{
    public static class PipeServer
    {
        ...
        public static void Start()
        {
            ...
            _serverThread.Start();
            Logger.LogMod("TheBadPlaceExecutor", "IPC Pipe Server started.");
        }
        ...
        private static void ServerLoop()
        {
            while (_isRunning)
            {
                    ...
                    using (var server = new NamedPipeServerStream(PipeName, PipeDirection.In))
                    {
                        ...
                        using (var reader = new StreamReader(server, Encoding.UTF8))
                        {
                            ...
                            if (!string.IsNullOrEmpty(script))
                            {
                                ...
                                Logger.LogMod("TheBadPlaceExecutor", "Received script via IPC.");
                            }
                        }
                    }
                }
                catch (Exception ex)
                {
                    if (_isRunning)
                    {
                        Logger.LogMod("TheBadPlaceExecutor", $"IPC Pipe Server error: {ex.Message}");
                    }
                }
            }
        }
